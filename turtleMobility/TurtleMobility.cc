...
void TurtleMobility::move()
{
    ...
    //ME
    ofstream out;
    if (lastUpdate == 0) { // zerar arquivo
        //out.open("C:/Users/moise/Documents/inet4/showcases/wireless/mobilitiesInfrastructure/results/" + outputFile);
        out.open("results/" + outputFile);
        //out << simTime() << " --- " << floor(simTime()) << " --- " << lastUpdate << " --- " << lastPosition << endl;
        out << simTime() << " --- " << lastPosition << endl;
    }
    else {
        out.open("results/" + outputFile, ios::app); // app evita sobrescrita
        /*if (simTime() - floor(simTime()) < 0.000001) {
            //out << simTime() << " --- " << floor(simTime()) << " --- " << lastUpdate << " --- " << lastPosition << endl;
            out << simTime() << " --- " << lastPosition << endl;
        }*/
        if (simTime() >= countTime) {
            countTime++;
            out << simTime() << " --- " << lastPosition << endl;
        }
    }
