...
void Way (int posI, int posJ, int posIb, int posJb, int &depth) {
    ...
    else if (m == d && posI < 3 && posI+1 != posIb) {
        out << "    <moveto x=\"" << posJ*100+20 << "\" y=\"" << (posI+1)*100+10 << "\"/>" << endl;
        Way(posI+1, posJ, posI, posJ, depth);
    }
    ...
    else if (m == r && posJ < 5 && posJ+1 != posJb) {
        out << "    <moveto x=\"" << (posJ+1)*100+20 << "\" y=\"" << posI*100+10 << "\"/>" << endl;
        Way(posI, posJ+1, posI, posJ, depth);
    }
    else {
...
int main() {
    ...
    for (int j = 0; j < 6; j++) { //line 0 and 2 --- left
        block[0][j].l = 1;
        block[1][j].l = 0;
        block[2][j].l = 2;
        block[3][j].l = 0;
    ...
    for (int j = 0; j < 6; j++) { //line 1, 2 and 3 --- right
        block[0][j].r = 0;
        block[1][j].r = 1;
        block[2][j].r = 2;
        block[3][j].r = 1;
    ...
    for (int i = 0; i < 4; i++) { //row 0, 2 and 4 --- down
        block[i][0].d = 1;
        block[i][1].d = 0;
        block[i][2].d = 2;
        block[i][3].d = 0;
    ...
    for (int i = 0; i < 4; i++) { //row 1, 2, 3 and 5 --- up
        block[i][0].u = 0;
        block[i][1].u = 1;
        block[i][2].u = 2;
        block[i][3].u = 1;
